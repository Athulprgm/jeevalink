...
export default function NotificationCenter() {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div>
        <h1 className="text-white text-xl font-black">Notification Center</h1>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
            ...
            {[
              ...
              <button key={key} onClick={() => setTab(key)}
                className={`flex-1 flex items-center justify-center gap-1.5 py-3 text-xs font-bold transition-colors cursor-pointer border-b-2 ${
                  tab === key ? 'border-red-500 text-red-400' : 'border-transparent text-slate-500 hover:text-slate-300'
                }`}>
              ...
              <>
                <div>
                  ...
                  <input value={broadcastForm.title} onChange={e => setBroadcastForm(f => ({ ...f, title: e.target.value }))}
                    ...
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 transition-colors" />
                </div>
                <div>
                  ...
                  <textarea value={broadcastForm.message} onChange={e => setBroadcastForm(f => ({ ...f, message: e.target.value }))}
                    ...
                    rows={4}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs placeholder:text-slate-600 focus:outline-none focus:border-blue-500/40 transition-colors resize-none" />
                </div>
                <div>
                  ...
                  <select value={broadcastForm.target} onChange={e => setBroadcastForm(f => ({ ...f, target: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-xs focus:outline-none focus:border-blue-500/40 transition-colors cursor-pointer">
                ...
                <button onClick={() => handleSend(broadcastForm, 'broadcast')} disabled={sending}
                  className="w-full py-3 bg-blue-500 hover:bg-blue-600 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
              ...
              <>
                ...
                <div>
                  ...
                  <input value={emergencyForm.title} onChange={e => setEmergencyForm(f => ({ ...f, title: e.target.value }))}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs focus:outline-none focus:border-red-500/40 transition-colors" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={emergencyForm.bloodGroup} onChange={e => setEmergencyForm(f => ({ ...f, bloodGroup: e.target.value }))}
                      className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-xs focus:outline-none focus:border-red-500/40 transition-colors cursor-pointer">
                ...
                <div>
                  ...
                  <textarea value={emergencyForm.message} onChange={e => setEmergencyForm(f => ({ ...f, message: e.target.value }))}
                    ...
                    rows={3}
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs placeholder:text-slate-600 focus:outline-none focus:border-red-500/40 transition-colors resize-none" />
                </div>
                <button onClick={() => handleSend({ ...emergencyForm, target: 'all' }, 'emergency')} disabled={sending}
                  className="w-full py-3 bg-red-500 hover:bg-red-600 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2 shadow-lg shadow-red-500/20">
              ...
              <>
                <div>
                  ...
                  <input value={scheduleForm.title} onChange={e => setScheduleForm(f => ({ ...f, title: e.target.value }))}
                    ...
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs placeholder:text-slate-600 focus:outline-none focus:border-purple-500/40 transition-colors" />
                </div>
                <div>
                  <label className="block text-slate-500 text-[10px] font-bold uppercase mb-1">Message</label>
                  <textarea value={scheduleForm.message} onChange={e => setScheduleForm(f => ({ ...f, message: e.target.value }))}
                    ...
                    className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-200 text-xs placeholder:text-slate-600 focus:outline-none focus:border-purple-500/40 transition-colors resize-none" />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <select value={scheduleForm.target} onChange={e => setScheduleForm(f => ({ ...f, target: e.target.value }))}
                      className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-xs focus:outline-none cursor-pointer">
                      <option value="all">All Users</option>
                  ...
                  <div>
                    ...
                    <input type="datetime-local" value={scheduleForm.scheduledAt} onChange={e => setScheduleForm(f => ({ ...f, scheduledAt: e.target.value }))}
                      className="w-full px-3 py-2.5 bg-white/5 border border-white/10 rounded-xl text-slate-300 text-xs focus:outline-none focus:border-purple-500/40 transition-colors cursor-pointer" />
                  </div>
                </div>
                <button onClick={() => handleSend(scheduleForm, 'scheduled')} disabled={sending}
                  className="w-full py-3 bg-purple-500 hover:bg-purple-600 text-white text-xs font-bold rounded-xl transition-colors cursor-pointer disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        <div className="bg-[#141929] border border-white/[0.06] rounded-2xl overflow-hidden">
          <div className="p-4 border-b border-white/[0.06]">
            <h3 className="text-white font-bold text-sm">Sent Notifications</h3>
              ...
              return (
                <motion.div key={n._id} initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                  ...
                  <div className="flex items-start justify-between mb-1.5">
                    <p className="text-slate-200 text-xs font-semibold flex-1 pr-2">{n.title}</p>
